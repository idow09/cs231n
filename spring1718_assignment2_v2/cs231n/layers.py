...
def batchnorm_backward_alt2(dout, cache):
    ...
    dx = 1 / N * gamma * (sample_var + eps) ** -0.5 * (
            N * dout - np.sum(dout, axis=0) - (x - sample_mean) * (sample_var + eps) ** -1 * np.sum(
        dout * (x - sample_mean), axis=0))
...
def layernorm_forward_2(x, gamma, beta, ln_param):
    """
    Forward pass for layer normalization.
    ...
    out, x_norm = normalize(x, mean, var, gamma, beta, eps)
...
def layernorm_backward(dout, cache):
    ...
    # dgamma = np.sum(dout * x_norm, axis=0)
    # dx_norm = dout * gamma
    # dvar = np.sum(dx_norm * (x - mean), axis=0) * (-1.0 / 2.0) / (np.sqrt(var + eps) * (var + eps))
    # dmean = (np.sum(dx_norm, axis=0) * (-1.0) / (np.sqrt(var + eps))) + dvar * np.sum(x - mean, axis=0) * (-2.0) / N
    # dx1 = dx_norm / np.sqrt(var + eps) + dvar * 2 * (x - mean) / N + dmean / N
    # dx = 1 / D * gamma * (var + eps) ** -0.5 * (D * dout - np.sum(dout, axis=0) - (x - mean) * (var + eps) ** -1
...
def max_pool_forward_naive(x, pool_param):
    ...
    H_new = 1 + (H - height) // stride
    W_new = 1 + (W - width) // stride

    out = np.empty((N, C, H_new, W_new))
    for n in range(N):
        for c in range(C):
            for i in range(H_new):
                hh = i * stride
                for j in range(W_new):
                    ww = j * stride
                    out[n, c, i, j] = np.max(x[n, c, hh:hh + height, ww:ww + width])
        ...
        for c in range(C):
            for i in range(H_out):
                ...
                for j in range(W_out):
                    ww = j * stride
                    max_idx = np.argmax(x[n, c, hh:hh + height, ww:ww + width])
                    dx[n, c, hh:hh + height, ww:ww + width].flat[max_idx] = dout[n, c, i, j]
    return dx
