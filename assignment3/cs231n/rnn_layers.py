...
def rnn_backward(dh, cache):
    ...
    """
    x0, _, Wx0, Wh0, b0, next_h0 = cache[0]
    ...
    for t in reversed(range(T)):
        temp_dx, temp_dh, temp_dWx, temp_dWh, temp_db = rnn_step_backward(temp_dh + dh[:, t, :], cache.pop())
        dx[:, t, :] = temp_dx
        dWx += temp_dWx
        dWh += temp_dWh
        db += temp_db
...
def lstm_forward(x, h0, Wx, Wh, b):
    ...
    temp_c = np.zeros((N, H))
    for t in range(T):
        temp_h, temp_c, temp_cache = lstm_step_forward(x[:, t, :], temp_h, temp_c, Wx, Wh, b)
        h[:, t, :] = temp_h
        cache.append(temp_cache)
    return h, cache
